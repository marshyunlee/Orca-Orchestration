...
import {ImportedTaskPanel} from './ImportedTaskPanel.js';
import type { ComponentBinding } from "../../../shared/collaborate.js";
import { CollaborateDetails } from "./CollaborateDetails.js";
import { useEffect, useReducer, useRef, useState } from "react";
import type { BoardSnapshot, BoardEdit, NodeContent, Assignment, MemberRef } from "../../../shared/board.js";
...
export function BoardView({onHistory}:{onHistory:()=>void}) {
  ...
  const [newMembers,setNewMembers]=useState<MemberRef[]>([]),[newCoordinator,setNewCoordinator]=useState("");
  const [inventory,setInventory]=useState<SessionInventory>({members:[],unavailable:[]}),[loadingSessions,setLoadingSessions]=useState(false);
  useEffect(()=>{if(!newGroup)return;let active=true;setLoadingSessions(true);void boardRequest<SessionInventory>("/api/sessions").then(value=>{if(active)setInventory(value);}).catch(error=>{if(active)setConnectionError(String(error));}).finally(()=>{if(active)setLoadingSessions(false);});return()=>{active=false;};},[newGroup]);
  ...
  async function createGroup() {
    setBusy(true);
    try {
      const snapshot=await boardRequest<BoardSnapshot>("/api/boards",{actionId:crypto.randomUUID(),title,members:newMembers,coordinatorIdentity:newCoordinator});
      dispatch({type:"save-succeeded",boardId:snapshot.id,snapshot,savedDrafts:{}});dispatch({type:"select",boardId:snapshot.id});setTitle("");setNewGroup(false);setNewMembers([]);setNewCoordinator("");
    }catch(error){setConnectionError(String(error));}finally{setBusy(false);}
  }
  return <main className="board-app">
    {historical && <section className="delivery-history" role="dialog" aria-label="Past delivery"><button onClick={()=>setHistorical(null)}>Close history</button><h2>{historical.title} · past delivery</h2><p>Recorded revision {historical.revision}. Historical contents are read-only.</p>{historical.nodes.map(item=><details key={item.id}><summary>{item.kind} · {item.title}</summary><pre className="preserve-lines">{Object.entries(item.content).map(([key,value])=>`${key}:\n${value}`).join("\n\n")}</pre></details>)}{historical.nodes.filter(item=>item.collaborate).map(item=><CollaborateDetails key={item.id} board={historical} node={item} readOnly onEdit={()=>{}} onSnapshot={()=>{}}/>)}{historical.nodes.filter(item=>item.imported).map(item=><ImportedTaskPanel key={item.id} board={historical} node={item} readOnly onEdit={()=>{}}/>)}<AttemptHistory boardId={historical.id} attempts={historical.attempts}/></section>}
    ...
    {newGroup && <form className="new-group" onSubmit={event=>{event.preventDefault();void createGroup();}}><label>Group name<input autoFocus value={title} onChange={event=>setTitle(event.target.value)}/></label>{loadingSessions && <p>Reading live session bindings…</p>}<SessionPicker inventory={inventory} members={newMembers} coordinatorIdentity={newCoordinator} onChange={(members,coordinator)=>{setNewMembers(members);setNewCoordinator(coordinator);}}/><button disabled={busy || !title.trim() || (newMembers.length>0 && !newCoordinator)}>Create group</button><button type="button" onClick={()=>setNewGroup(false)}>Cancel</button></form>}
