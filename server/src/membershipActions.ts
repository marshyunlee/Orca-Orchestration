...
export function createMembershipActions(store:BoardStore, verify=verifyGroupMember){return {
 ...
 async finish(boardId:string,actionId:string,identity:string,token:string,receipt:Record<string,unknown>){
  ...
  return store.update(boardId,board.revision,`membership-receipt-${token}-${createHash("sha256").update(JSON.stringify(receipt)).digest("hex")}`,current=>{
