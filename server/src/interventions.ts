 ...
 return {
  async begin(boardId:string,actionId:string,identity:string):Promise<{operation:NativeOperation|null;token:string|null;waiting?:string}>{
   ...
   await store.update(boardId,before.revision,`effect-${token}`,board=>{
     ...
     if(!operation){current.phase="applied";return board;}
  ...
  async finish(boardId:string,actionId:string,token:string,receipt:NativeReceipt):Promise<BoardSnapshot>{
   ...
   return store.update(boardId,before.revision,`effect-receipt-${token}`,board=>{
    ...
    if(operation.kind==="send-guidance" && attempt){
    ...
    if(operation.kind==="stop-worker" && attempt){
      ...
      if(node.revision<=attempt.nodeRevision){action.phase="failed";action.error="Unchanged failed inputs require native retry; revise the task or reconcile the native retry before launch";return board;}
