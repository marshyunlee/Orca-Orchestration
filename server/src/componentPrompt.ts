...
export function componentActionPrompt(boardId:string,node:BoardNode,action:ActionRecord,cli:string,url:string):string {
  return [
    ...
    `For guidance, send the saved revision to the exact active child Dispatch and report acknowledgment separately. Restricted workers cannot run Orca to consume their inbox: when the wake is only a check-inbox notice, verify the recorded terminal/incarnation and deliver the same nonsecret body to that exact terminal, retaining both receipts. For stop, settle each active child using native stop/recovery, archive evidence, and preserve this master. A new node/gate revision needs its corresponding approval before rerun.`,
