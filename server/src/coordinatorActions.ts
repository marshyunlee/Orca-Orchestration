    ...
    async deliver(boardId:string,actionId:string):Promise<void>{
      ...
      try {
        ...
        const basePrompt=action.kind==="import-control"?importedControlPrompt(board,action):component?componentActionPrompt(boardId,component,action,command,process.env.ORCA_BOARD_URL??`http://127.0.0.1:${process.env.PORT??8787}`):[
          ...
          `CLI: ${command}. Server: ${process.env.ORCA_BOARD_URL??`http://127.0.0.1:${process.env.PORT??8787}`}. Use --help once for typed commands; status for supervision; append --compact to mutation commands. Read full content only when needed.`,
          `Board artifacts: ${join(store.root,boardId)}. Keep all discussion and evidence here.`,
          ...(["collect-work"].includes(action.kind)?[`For collect-work: read collection.requests and saved context artifacts; run collection-send --request <id> for each pending member. Publish your own summary with collection-publish. Members answer at their next safe checkpoint. Synthesize available goals, accepted scope and open questions into a spec proposal; identify missing replies. Never call group-init, rebind a Run, create a summary Dispatch or approve the spec.`]:[]),
          ...(["discuss"].includes(action.kind)?[`For discuss: initialize/attach the shared group via boardctl group-init; follow the group skill's finite rounds, collect attributed contributions, publish its canonical document, and propose the resulting spec. Do not infer agreement from silence.`]:[]),
          ...(["generate-tasks"].includes(action.kind)?[`For generate-tasks: propose task nodes, exact group-member or new-worker assignments, and edges from the Run root using the approved spec.`]:[]),
          ...(["review-graph"].includes(action.kind)?[`For review-graph: inspect current spec, graph, assignments, results; publish an expected-deliverable Preview with examples, behavior, acceptance criteria, and open assumptions. Do not execute tasks to create Preview.`]:[]),
          ...(["start", "resume"].includes(action.kind)?[`For start or resume: execute the claimed action, then supervise through boardctl launch; choose ready waves and process native questions/results. Publish questions and exact attempt observations to the board. Do not use raw native starts for board-owned tasks. If starts are paused, leave active work running and idle after recording its outcomes; a new human action will wake you.`]:[]),
          ...(["guidance", "stop-rerun", "answer-question", "members", "handover", "reconcile"].includes(action.kind)?[`For guidance, stop-rerun, answer-question, members, handover, or reconcile: use boardctl execute-action. Preserve existing group sessions. After cooperative stop, wait for the member acknowledgment and execute the stop action again; launch the revised node only after rerunReady. Process queued human actions during your supervision loop, not only terminal prompts.`]:[]),
