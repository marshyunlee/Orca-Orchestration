 ...
 return {
  ...
  async prepareDelivery(boardId:string,requestId:string,identity:string):Promise<NativeOperation>{
   ...
   const body=[`At your next safe checkpoint, contribute a bounded summary to existing-session board ${board.id}. This adds visibility to your current work; preserve your task, Run, session, mailbox and approval scope. Do not call group-init or create a Dispatch for this request.`,
    `CLI: ${process.env.ORCA_BOARD_CLI??'boardctl'} collection-publish --compact --board ${board.id} --request ${request.id} --file <summary.json> --terminal ${request.member.terminalHandle} --url ${url} --token-file ${join(runtime,`token-${new URL(url).port||80}`)}.`,
    ...
    'Publish once when safe, then continue your original work; no separate acknowledgment message or summary round is required. An enqueue receipt is not a response or evidence of completion.'].join('\n');
