#!/usr/bin/env node
import {compactBoardOutput} from "./boardOutput.js";
...
function printOutput(serialized:string):void {
 console.log(args.includes('--compact') || args[0]==='status'?JSON.stringify(compactBoardOutput(JSON.parse(serialized))):serialized);
}
...
async function main():Promise<void>{
  ...
  owner-reconcile --action <id> --evidence <text>  Resolve a stale action without replay
  ...
  status                       Compact revisions, ownership, actions and attempts
  read                         Full board, revisions, digests, queued actions and attempts
...
Use status for compact supervision. Append --compact to mutation commands to omit full node content and historical discussion from returned boards. Read without --compact preserves the full JSON contract.
...
Use --terminal <your-own-handle> when the tool shell lacks inherited Orca identity, and --token-file <private-runtime/token-PORT> for a nondefault runtime. Verify that the selected terminal is your actual session; never borrow another session identity. Native operations run only for the selected coordinator. Unknown receipts require reconciliation; never repeat an unknown mutation blindly.`);return;
  ...
  if(args[0]==="native"){
    ...
    const receipt=await executeNativeOperation(operation,caller,{callerTerminal:callerHandle});printOutput(JSON.stringify(receipt));if(receipt.phase!=="applied")process.exitCode=1;return;
  }
  if(args[0]==="create"){
    ...
    printOutput(JSON.stringify({board:created,url,role:"coordinator",collectionQueued:true}));return;
  }
  if(args[0]==="resume"){
    ...
    printOutput(JSON.stringify({board:selected,url,...role}));return;
  }
  const id=option("--board");
  const board=await request<BoardSnapshot>(`/api/boards/${encodeURIComponent(id)}`);
  if(args[0]==="read" || args[0]==="status"){printOutput(JSON.stringify(board,null,2));return;}
  if(args[0]==="owner-claim" || args[0]==="owner-finish" || args[0]==="owner-reconcile"){
    ...
    printOutput(JSON.stringify(await request(`/api/boards/${id}/collection/${args[0]}`,{identity:member.identity,actionId:option("--action"),...(args[0]==="owner-finish"?{stage:option("--stage"),evidence:option("--evidence")}:args[0]==="owner-reconcile"?{evidence:option("--evidence")}:{})})));return;
  }
  if(args[0]==="collect"){
    await selectedCaller(board);printOutput(JSON.stringify(await request(`/api/boards/${id}/collection/refresh`,{})));return;
  }
  if(args[0]==="collection-publish"){
    ...
    printOutput(JSON.stringify(await request(`/api/boards/${id}/collection/publish`,{identity:entry.member.identity,requestId:entry.id,summary:JSON.parse(await readFile(option("--file"),"utf8"))})));return;
  }
  if(args[0]==="collection-send" || args[0]==="collection-reconcile"){
    ...
    if(args[0]==="collection-reconcile"){
      if(!entry.requestId){
        ...
        printOutput(JSON.stringify(await request(`/api/boards/${id}/collection/delivery`,{identity:caller.identity,requestId:entry.id,receipt:recovered})));return;
      }
    ...
    printOutput(JSON.stringify(await request(`/api/boards/${id}/collection/delivery`,{identity:caller.identity,requestId:entry.id,receipt})));return;
  }
  if(args[0]==="attest-stop"){
    const member=board.members.find(member=>member.terminalHandle===callerHandle);
    if(!member)throw new Error("Run stop acknowledgment from the assigned member session");
    printOutput(JSON.stringify(await request(`/api/boards/${id}/member/stopped`,{identity:member.identity,attemptId:option("--attempt"),evidence:option("--evidence")})));return;
  }
  if(args[0]==="component-refresh" || args[0]==="component-approve" || args[0]==="component-result" || args[0]==="component-claim" || args[0]==="component-finish"){
    ...
    printOutput(JSON.stringify(await request(`/api/boards/${id}/components/${nodeId}/${operation}`,data)));return;
  }
  ...
  if(args[0]==="claim"){
    printOutput(JSON.stringify(await request(`/api/boards/${id}/coordinator/claim`,{identity:caller.identity,actionId:option("--action")})));return;
  }
  if(args[0]==="publish"){
    const proposal=JSON.parse(await readFile(option("--file"),"utf8"));
    printOutput(JSON.stringify(await request(`/api/boards/${id}/coordinator/publish`,{identity:caller.identity,actionId:option("--action"),baseRevision:Number(option("--base-revision")),proposal})));return;
  }
  if(args[0]==="reconcile"){
    ...
    if(data.membershipToken && board.discussionGroupId){
      ...
      printOutput(JSON.stringify(await request(`/api/boards/${id}/coordinator/membership-receipt`,{identity:caller.identity,actionId:action.id,token:data.membershipToken,receipt})));return;
    }
    ...
    printOutput(JSON.stringify(await request(`/api/boards/${id}/coordinator/${endpoint}`,{identity:caller.identity,actionId:action.id,token,receipt})));return;
  }
  if(args[0]==="execute-action"){
    ...
    if(action.kind==="members"){
      ...
      printOutput(JSON.stringify(await request(`/api/boards/${id}/coordinator/membership-receipt`,{identity:caller.identity,actionId:action.id,token:change.token,receipt})));return;
    }
    ...
    if(!prepared.operation){printOutput(JSON.stringify(prepared));return;}
    const receipt=await executeNativeOperation(prepared.operation,caller,{callerTerminal:callerHandle});
    printOutput(JSON.stringify(await request(`/api/boards/${id}/coordinator/action-receipt`,{identity:caller.identity,actionId:action.id,token:prepared.token,receipt})));
  ...
  if(args[0]==="launch"){
    ...
    if(node.collaborate){
      printOutput(JSON.stringify(await request(`/api/boards/${id}/edit`,{baseRevision:board.revision,actionId,operation:{kind:"component-start",nodeId:node.id,body:"Execute the approved component scope using collaborate."}})));return;
    }
    ...
    for(let step=0;step<2;step++){
      ...
      printOutput(JSON.stringify({actionId,operation:operation.operation.kind,receipt,attempt:updated.attempts.find(attempt=>attempt.id===permit.attemptId)}));
  ...
  if(args[0]==="group-init"){
    ...
    printOutput(JSON.stringify(await callGroupHelper(["attach","--group",groupId,"--actor",caller.identity])));return;
  }
